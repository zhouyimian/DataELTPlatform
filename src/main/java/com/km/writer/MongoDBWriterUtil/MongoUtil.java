package com.km.writer.MongoDBWriterUtil;

import com.km.common.exception.DataETLException;
import com.km.common.util.Configuration;
import com.km.reader.mongodbReaderUtil.KeyConstant;
...
public class MongoUtil {
    ...
    public static MongoClient initMongoClient(Configuration conf) {
        ...
        if(addressList == null || addressList.size() <= 0) {
            throw DataETLException.asDataETLException(MongoDBWriterErrorCode.ILLEGAL_VALUE,"不合法参数");
        }
        try {
            return new MongoClient(parseServerAddress(addressList));
        } catch (UnknownHostException e) {
           throw DataETLException.asDataETLException(MongoDBWriterErrorCode.ILLEGAL_ADDRESS,"不合法的地址");
        } catch (NumberFormatException e) {
           throw DataETLException.asDataETLException(MongoDBWriterErrorCode.ILLEGAL_VALUE,"不合法参数");
        } catch (Exception e) {
            throw DataETLException.asDataETLException(MongoDBWriterErrorCode.UNEXCEPT_EXCEPTION,"未知异常");
        }
    }
