package com.km.data.writer.hdfsWriterUtil;
...
import com.km.data.writer.HdfsWriter;
    ...
    public void getFileSystem(String defaultFS, Configuration taskConfig) {
        ...
        if (null == fileSystem || null == conf) {
            String message = String.format("获取FileSystem失败,请检查HDFS地址是否正确: [%s]",
                    "message:defaultFS =" + defaultFS);
            LOG.error(message);
            throw DataETLException.asDataETLException(HdfsWriterErrorCode.CONNECT_HDFS_IO_ERROR, message);
        }
    }
...
//                MutablePair<List<Object>, Boolean> transportResult =  transportOneRecord(object,columns);
