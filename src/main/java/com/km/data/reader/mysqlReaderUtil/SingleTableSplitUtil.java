package com.km.data.reader.mysqlReaderUtil;
...
public class SingleTableSplitUtil {
    ...
    public static List<Configuration> splitSingleTable(DataBaseType dataBaseType,
        ...
        Pair<Object, Object> minMaxPK = getPkRange(dataBaseType,configuration);
        if (null == minMaxPK) {
            throw DataETLException.asDataETLException(DBUtilErrorCode.ILLEGAL_SPLIT_PK,
                    "根据切分主键切分表失败. 平台 仅支持切分主键为一个,并且类型为整数或者字符串类型. 请尝试使用其他的切分主键");
        }
...
//        jdbcURL = jdbcURL.replace("\"]","");
     ...
     */
    private static Pair<Object, Object> checkSplitPk(Connection conn, String pkRangeSQL, String table,
        ...
        try {
            try {
                rs = DBUtil.query(conn, pkRangeSQL);
            } catch (Exception e) {
                throw e;
            }
            ...
            throw DataETLException.asDataETLException(DBUtilErrorCode.ILLEGAL_SPLIT_PK, "DataX尝试切分表发生错误. 请检查您的配置并作出修改.", e);
        } finally {
