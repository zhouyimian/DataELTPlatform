package com.km.data.reader.mongodbReaderUtil;
...
public class MongoUtil {
    ...
    public static MongoClient initMongoClient(Configuration conf) {
        ...
        if(addressList == null || addressList.size() <= 0) {
            throw DataETLException.asDataETLException(MongoDBReaderErrorCode.ILLEGAL_VALUE,"不合法参数");
        }
        try {
            return new MongoClient(parseServerAddress(addressList));
        } catch (UnknownHostException e) {
            throw DataETLException.asDataETLException(MongoDBReaderErrorCode.ILLEGAL_ADDRESS,"不合法的地址");
        } catch (NumberFormatException e) {
            throw DataETLException.asDataETLException(MongoDBReaderErrorCode.ILLEGAL_VALUE,"不合法参数");
        } catch (Exception e) {
            throw DataETLException.asDataETLException(MongoDBReaderErrorCode.UNEXCEPT_EXCEPTION,"未知异常");
        }
    }
