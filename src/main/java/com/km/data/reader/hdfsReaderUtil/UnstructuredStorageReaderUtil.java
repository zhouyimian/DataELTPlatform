...
public class UnstructuredStorageReaderUtil {
    ...
    public static void validateEncoding(Configuration readerConfiguration) {
        ...
        try {
            ...
            throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.ILLEGAL_VALUE,
                    String.format("不支持您配置的编码格式 : [%s]", encoding), uce);
        } catch (Exception e) {
            throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.CONFIG_INVALID_EXCEPTION,
    ...
    public static void validateFieldDelimiter(Configuration readerConfiguration) {
        ...
        if (null == delimiterInStr) {
            throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE,
    ...
    public static void validateColumn(Configuration readerConfiguration) {
        ...
        if (null == columns || columns.size() == 0) {
            throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.REQUIRED_VALUE, "您需要指定 columns");
        }
        // handle ["*"]
        ...
        if (null != columns && columns.size() != 0) {
            for (Configuration eachColumnConf : columns) {
                ...
                if (null == columnIndex && null == columnValue) {
                    throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.NO_INDEX_VALUE,
                            "由于您配置了type, 则至少需要配置 index 或 value");
                }

                if (null != columnIndex && null != columnValue) {
                    throw DataETLException.asDataETLException(UnstructuredStorageReaderErrorCode.MIXED_INDEX_VALUE,
    ...
    public static void readFromStream(InputStream inputStream, String context,
                                      Configuration readerSliceConfig, Channel channel) {
    ...
    public static void doReadFromStream(BufferedReader reader, String context,
        ...
        try {
            ...
            while ((parseRows = UnstructuredStorageReaderUtil
                ...
                UnstructuredStorageReaderUtil.transportOneRecord(channel, column, parseRows);
            }
        } catch (UnsupportedEncodingException uee) {
            throw DataETLException
                    .asDataETLException(
    ...
    public static Record transportOneRecord(Channel channel,
        ...
        return transportOneRecord(channel, column, sourceLine);
    }
}
