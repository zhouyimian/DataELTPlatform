package com.km.data.reader.hdfsReaderUtil;
...
import com.km.data.reader.HdfsReader;
...
public class DFSUtil {
    ...
    public HashSet<String> getHDFSAllFiles(String hdfsPath) {
        ...
        try {
            ...
            LOG.error(message);
            throw DataETLException.asDataETLException(HdfsReaderErrorCode.PATH_CONFIG_ERROR, e);
        }
    }
    ...
    public InputStream getInputStream(String filepath) {
        ...
        try {
            ...
            throw DataETLException.asDataETLException(HdfsReaderErrorCode.READ_FILE_ERROR, message, e);
        }
    }
...
//                        , sourceOrcFilePath);
//                LOG.error(message);
//                throw DataETLException.asDataETLException(HdfsReaderErrorCode.READ_FILE_ERROR, message);
//            }
//        } else {
//            String message = String.format("请确认您所读取的列配置正确！columnIndexMax 小于0,column:%s", JSON.toJSONString(column));
//            throw DataETLException.asDataETLException(HdfsReaderErrorCode.BAD_CONFIG_VALUE, message);
//        }
//    }
    public void orcFileStartRead(String sourceOrcFilePath, Configuration readerSliceConfig,
                                 Channel channel) {
        ...
        if (columnIndexMax >= 0) {
            ...
            try {
                ...
                while (reader.next(key, value)) {
                    ...
                    transportOneRecord(column, recordFields, channel,
                            isReadAllColumns, nullFormat);
                }
                reader.close();
            } catch (Exception e) {
                String message = String.format("从orcfile文件路径[%s]中读取数据发生异常，请联系系统管理员。"
    ...
    private int getMaxIndex(List<ColumnEntry> columnConfigs) {
        ...
        for (ColumnEntry columnConfig : columnConfigs) {
            ...
            if (columnIndex != null && columnIndex < 0) {
                ...
                LOG.error(message);
                throw DataETLException.asDataETLException(HdfsReaderErrorCode.CONFIG_INVALID_EXCEPTION, message);
    ...
    private JSONObject transportOneRecord(List<ColumnEntry> columnConfigs, List<Object> recordFields
            , Channel channel, boolean isReadAllColumns, String nullFormat) {
        ...
        try {
            ...
            if (isReadAllColumns) {
                ...
                for (Object recordField : recordFields) {
                    ...
                    column.put("rawDate",columnValue);
                    array.add(column);
                }
            } else {
                for (ColumnEntry columnConfig : columnConfigs) {
...
//                                                    .asDate());
        ...
        try {
            ...
            while (reader.next(key, value)) {
                if (StringUtils.isNotBlank(value.toString())) {
                    UnstructuredStorageReaderUtil.transportOneRecord(channel,
                            readerSliceConfig, value.toString());
                }
            }
        } catch (Exception e) {
            String message = String.format("SequenceFile.Reader读取文件[%s]时出错", sourceSequenceFilePath);
            LOG.error(message);
            throw DataETLException.asDataETLException(HdfsReaderErrorCode.READ_SEQUENCEFILE_ERROR, message, e);
        } finally {
    ...
    public void rcFileStartRead(String sourceRcFilePath, Configuration readerSliceConfig,
        ...
        try {
            ...
            LOG.error(message);
            throw DataETLException.asDataETLException(HdfsReaderErrorCode.READ_RCFILE_ERROR, message, e);
        } finally {
            try {
